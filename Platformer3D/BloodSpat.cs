...
{
    public class BloodSpat : GameObject3D
    {
        private int lifeTimer;
        private new Vector3 velocity;

        public BloodSpat(Vector3 spawnPos, float newScale)
        {
            lifeTimer = 1000;

            lighting = false;

            _model = Game1.Instance.Content.Load<Model>("Models\\blood");
        ...
        public BloodSpat(Vector3 spawnPos, float newScale, Vector2 dir)
        {
            lifeTimer = 1000;
        ...
        public override bool Update(float dTime)
        {
            ...
            rotationMatrix *= Matrix.CreateRotationX(MathHelper.ToRadians(-rotation));
            if (lifeTimer > 0)
            {
                lifeTimer -= (int)dTime;
                scale += dTime * .01f;
